...
def notAquake(q):
    if q["properties"]["type"]== "earthquake":
        return False
    return True
#creates a list called events that filters the features data using the not a quake function
events = list(filter(notAquake, data["features"]))
print(f"The total non quake events: {len(events)}")
#prints the first 10 entries in our not a quake list
for i in range(0,10):
